...
def score_dataframe(df: pd.DataFrame, vega_max: float, theta_max: float, delta_min: float, min_skew: float) -> pd.DataFrame:
    ...
    d["Best Score"] = d[score_cols].max(axis=1)
    d["Play Type"] = d[score_cols].idxmax(axis=1).str.replace(" Score", "", regex=False).str.upper()
    ...
    d["Regime Score"] = [d.loc[i, map_score.get(pt, "Best Score")] for i, pt in d["Play Type"].items()]
